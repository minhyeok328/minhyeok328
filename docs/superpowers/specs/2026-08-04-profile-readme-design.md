...
다음 내용을 한국어 3개 문단으로 작성한다. 섹션명 `About Me`는 영어로 유지한다.
...
README의 서비스 소개 문단은 다음과 같이 한국어로 작성한다.
...
표의 헤더와 구분선은 모두 4열로 맞춘다. 각 열의 구분선이 누락되면 GitHub가 표로 렌더링하지 않으므로 `| --- | --- | --- | --- |`를 유지한다.
...
README의 분류 설명은 한국어로 작성한다. `Primary`와 `Project experience` 표기 자체는 유지한다.

> `Primary`는 반복적으로 직접 구현하고 검증한 핵심 기술이며, `Project experience`는 이전 프로젝트에서 직접 사용했거나 인터페이스 경계에서 연동한 기술입니다.

팀 기술 스택에 존재했을 뿐 실제로 다루지 않은 기술은 표기하지 않는다.
...
- 영어 섹션명, 표 헤더와 기술명은 유지하고 설명형 본문은 한국어로 작성한다.
- 이번 문구 보정 범위는 `About Me` 3개 문단, HumouR 서비스 소개, PICKLE 내부 평가 설명과 Tech Stack 분류 설명이다.
...
7. GitHub Markdown 표와 HTML이 깨지지 않으며, 모든 표의 헤더 열 수와 구분선 열 수가 일치한다.
8. 기존 `codex_subagent`는 `Other Work`에서 유지된다.
9. GitHub 통계는 페이지 하단의 보조 정보로만 남는다.
10. 사실성 가드레일에 위배되는 문장이 없다.
11. `About Me`, HumouR 서비스 소개, PICKLE 내부 평가 설명과 Tech Stack 분류 설명이 한국어로 표시된다.
